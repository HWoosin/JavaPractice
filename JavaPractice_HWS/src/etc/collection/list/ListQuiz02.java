...
public class ListQuiz02 {
	...
	public static void main(String[] args) {
        ...
        */
		...
		List<User> dao = new ArrayList<>();
		
		User hong = new User("홍길동",30);
		User kim = new User("김개똥",20);
		User lee = new User("이잉기",80);
		 ...
		 * 리스트의 크기를 마음대로 늘리거나 줄일 수 없다.
		 * 향상 for문은 반복문을 시작할 때 구조적으로 컬렉션의
		 * 크기 및 사용 가능한 요소들의 개수를 항상 파악하고 있기 때문에
		 * 값의 변경이 일어나면 예외를 발생시키도록 설계되어있다.
		 */
		
		for(int i=0; i<dao.size(); i++) {
			if((dao.get(i)).getName().equals("홍길동")) {
				dao.remove(i);
