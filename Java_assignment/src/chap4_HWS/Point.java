...
public class Point {
	...
	public void showAllPoints(List<Student> all) {//학생 객체들이 들어있는 리스트가 와야죠?
		int classSum = 0;
		showPointUI();
		for(Student s : all) {
			s.outputStuInfo();
		...
		System.out.println("반 평균: " + classAvg);
	...
	public void deletePoint(List<Student> students) {
		...
		for(int i=0; i<students.size();i++) {
			if(students.get(i).getStuId().equals(stuId)) {
				System.out.println(stuId + "님의 정보를 삭제합니다.[Y / N]");
				System.out.print(">");
